fn belongs_in_set(x: usize, y: usize) -> bool {
    ...
    for i in 0..10 {
        let temp_real_component = real_component * real_component - imaginary_component * imaginary_component + x;
...
pub fn draw(width: usize, height: usize) -> Vec<Vec<usize>> {
    ...
    for x in 0..width {
        canvas[x] = Vec::new();
        for y in 0..height {
            let belongs = belongs_in_set(x / MAGNIFICATION_FACTOR - PAN_X, y / MAGNIFICATION_FACTOR - PAN_Y);
